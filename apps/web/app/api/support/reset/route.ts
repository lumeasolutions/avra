...
export async function POST(req: NextRequest) {
  ...
  // En plus, sur cette action DESTRUCTIVE, on EXIGE un header Origin présent ET
  // correspondant à l'hôte (un Origin absent n'est pas un laissez-passer).
  const origin = req.headers.get('origin');
  const host = req.headers.get('host');
  let originOk = false;
  try {
    originOk = !!origin && !!host && new URL(origin).host === host;
  } catch {
    originOk = false;
  }
  if (!originOk) {
    return NextResponse.json({ error: 'Origine non autorisée pour cette action.' }, { status: 403 });
  }
