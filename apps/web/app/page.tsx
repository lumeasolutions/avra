  ...
  return (
    <>
      ...
      <section className="section">
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(320px,1fr))', gap: '24px' }}>
            {[
              { icon: '📁', title: 'Dossiers clients centralisés', desc: "Toutes les infos, documents, photos et échanges d'un dossier en un seul endroit. Fini les fichiers éparpillés.", badge: 'Essentiel' },
              { icon: '🧾', title: 'Facturation intelligente', desc: 'Devis, acomptes, factures et avoirs générés en 2 clics. Conformes à la réglementation française.', badge: 'Populaire' },
              { icon: '🤖', title: 'IA Photo-réalisme', desc: "Générez des rendus photo-réalistes de cuisines et d'aménagements directement depuis vos plans et descriptions.", badge: 'IA' },
              { icon: '📅', title: 'Planning & chantiers', desc: 'Planifiez poses, livraisons et interventions avec un calendrier visuel partagé avec votre équipe.', badge: 'Pro' },
              { icon: '📦', title: 'Gestion de stock', desc: 'Suivez vos matériaux, composants et commandes fournisseurs. Alertes de rupture automatiques.', badge: 'Pro' },
              { icon: '✍️', title: 'Signature électronique', desc: 'Faites signer vos devis et contrats en ligne en quelques secondes. Juridiquement valable.', badge: 'Inclus' },
            ].map((f) => (
        ...
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          <div style={{ color: 'var(--white)' }} className="reveal">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
              {['Rendus FLUX Pro ultra-réalistes', 'Chat IA contextuel sur chaque dossier', 'Suggestions automatiques de matériaux', 'Export HD pour présentations clients'].map((item) => (
          ...
          <div className="reveal" style={{ background: 'linear-gradient(135deg,rgba(201,169,110,.12),rgba(48,64,53,.5))', borderRadius: 20, padding: '2rem', border: '1px solid rgba(201,169,110,.2)' }}>
            ...
            {[
              ...
              <div key={i} style={{ padding: '12px 16px', borderRadius: 12, marginBottom: '12px', background: msg.who === 'user' ? 'rgba(255,255,255,.07)' : 'rgba(201,169,110,.12)', border: `1px solid ${msg.who === 'user' ? 'rgba(255,255,255,.1)' : 'rgba(201,169,110,.2)'}`, textAlign: msg.who === 'user' ? 'right' : 'left' }}>
      ...
      <section className="section" style={{ background: 'var(--cream-light)' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          <div className="reveal" style={{ background: 'var(--white)', borderRadius: 20, padding: '2rem', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border)' }}>
            ...
            {[
              ...
              <div key={s.stage} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                ...
                <div style={{ flex: 1, height: 24, background: 'var(--cream-dark)', borderRadius: 6, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${Math.min(100, s.count * 2)}%`, background: s.color, borderRadius: 6, transition: 'width .8s ease' }} />
                </div>
