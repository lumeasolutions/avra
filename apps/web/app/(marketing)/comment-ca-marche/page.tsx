...
export const metadata: Metadata = {
  title: 'Comment ça marche — AVRA, opérationnel en 5 minutes',
  description:
    "Découvrez comment démarrer avec AVRA en 5 étapes simples. Inscription, configuration de votre profil métier, import de vos données, et vous êtes opérationnel. Essai gratuit 14 jours.",
...
const steps = [
  {
    ...
    detail: "Email + mot de passe, c'est tout. Aucune information de paiement demandée pour démarrer l'essai.",
  },
  {
    num: '02',
    icon: '🎯',
    ...
    detail: 'L\'assistant de configuration vous guide étape par étape. 5 minutes suffisent pour avoir une app aux couleurs de votre entreprise.',
  },
  {
    num: '03',
    icon: '📥',
    ...
    detail: 'Imports guidés avec vérification et correction automatique des erreurs. Votre équipe retrouve ses repères immédiatement.',
  },
  {
    num: '04',
    icon: '✨',
    ...
    detail: 'Tout est intuitif. Pas besoin de formation. La plupart des utilisateurs créent leur premier devis en moins de 10 minutes.',
  },
  {
    num: '05',
    icon: '📈',
    ...
    detail: 'Accédez aux statistiques, paramétrez les automatisations et invitez vos collaborateurs. AVRA grandit avec votre entreprise.',
  },
];
...
const faqs = [
  ...
  {
    q: "AVRA fonctionne-t-il hors connexion ?",
    a: "AVRA nécessite une connexion internet pour fonctionner pleinement. Certaines fonctionnalités de consultation sont disponibles en mode dégradé hors ligne.",
  },
  {
    q: "Y a-t-il un support si j'ai des questions ?",
    a: "Oui. Support par email, chat en direct pendant les heures ouvrées, et une base de connaissance complète. Les plans Pro et Enterprise bénéficient d'un support prioritaire.",
  },
];
    ...
    <>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '60px 5%' }}>
          <div className="section-label" style={{ margin: '0 auto 1.5rem' }}>Démarrage rapide</div>
          ...
          <p style={{ color: 'rgba(255,255,255,.85)', fontSize: '1.15rem', maxWidth: 560, margin: '0 auto 2rem' }}>
            Pas de formation longue, pas de technicien requis. Suivez ces 5 étapes simples et commencez à gagner du temps dès aujourd&apos;hui.
          </p>
          <a href="/register"><button className="btn-primary">Démarrer maintenant — C&apos;est gratuit →</button></a>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <div style={{ position: 'relative' }}>
            {/* Timeline line */}
            <div style={{
              position: 'absolute', left: '60px', top: 0, bottom: 0, width: 2,
            ...
            {steps.map((step, i) => (
              <div key={step.num} className="reveal" style={{
                ...
                <div style={{
                  ...
                  color: 'var(--green-deep)', boxShadow: '0 4px 16px rgba(201,169,110,.4)',
                  zIndex: 1,
                }}>
                  {step.num}
                </div>
                ...
                <div className="card" style={{ flex: 1 }}>
                  ...
                  <div style={{
                    padding: '12px 16px', background: 'var(--cream-light)',
                    borderRadius: 8, borderLeft: '3px solid var(--gold)',
                    fontSize: '.9rem', color: 'var(--text-muted)',
                  }}>
                    💡 {step.detail}
                  </div>
                </div>
      ...
      {/* Garanties */}
      <section className="section section-centered" style={{ background: 'var(--cream-dark)' }}>
        <div className="container">
          <div className="section-label" style={{ margin: '0 auto 1.5rem' }}>Nos garanties</div>
          <h2 className="section-title">Zéro risque, zéro surprise</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: '24px', marginTop: '3rem' }}>
            {[
              { icon: '🆓', title: '14 jours gratuits', desc: "Accès complet à toutes les fonctionnalités Pro. Sans carte bancaire." },
              { icon: '🚫', title: 'Sans engagement', desc: "Résiliez quand vous voulez, en 1 clic. Pas de frais cachés." },
              { icon: '📤', title: 'Export de données', desc: "Vos données vous appartiennent. Exportez tout en CSV ou Excel à tout moment." },
              { icon: '🔒', title: 'Données sécurisées', desc: "Hébergement en France, chiffrement TLS, sauvegardes quotidiennes." },
            ].map((g) => (
              <div key={g.title} className="card reveal" style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>{g.icon}</div>
                <h4 style={{ marginBottom: '.5rem' }}>{g.title}</h4>
                <p style={{ fontSize: '.9rem' }}>{g.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: 780 }}>
          <div className="section-centered" style={{ marginBottom: '3rem' }}>
            <div className="section-label" style={{ margin: '0 auto 1.5rem' }}>FAQ</div>
            <h2 className="section-title">Questions fréquentes</h2>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {faqs.map((faq) => (
              <div key={faq.q} className="card reveal" style={{ padding: '1.5rem' }}>
      ...
      <section className="section section-centered" style={{ background: 'linear-gradient(135deg,var(--green-deep),var(--green))' }}>
        <div className="container">
          <h2 style={{ color: 'var(--white)', marginBottom: '1.5rem' }}>Vous avez encore des questions ?</h2>
          <p style={{ color: 'rgba(255,255,255,.75)', maxWidth: 500, margin: '0 auto 2.5rem' }}>
            Notre équipe est disponible par chat ou email pour vous accompagner dans vos premiers pas sur AVRA.
          </p>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="/register"><button className="btn-primary">Essayer gratuitement →</button></a>
            <a href="mailto:contact@avra.fr"><button className="btn-secondary">Contacter l&apos;équipe</button></a>
          </div>
        </div>
      </section>
