...
export default function PortailMenuisierPage() {
  ...
  const stats = useMemo(() => {
    ...
    const retouchesOuvertes = dossiersSignes.filter(d => (d.confirmations ?? []).some(c => !c.validee)).length;
    return { ca, fabricationsEnCours, livraisonsPrevues, chantiersBloques, retouchesOuvertes };
  ...
  return (
    <div style={{ padding: '28px 24px 28px 0', fontFamily: "'Segoe UI', Arial, sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
        {[
          ...
          <div key={k.label} style={{
            ...
            <div style={{ fontSize: 11, color: '#8B6950', fontWeight: 600 }}>{k.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 28 }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0F2540' }}>📋 FABRICATIONS EN COURS</h3>
            <Link href="/dossiers" style={{ fontSize: 12, color: '#7B4F2E', fontWeight: 600, textDecoration: 'none' }}>Voir tous →</Link>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {dossiers.slice(0, 6).map(d => renderDossierItem(d))}
            {dossiers.length === 0 && (
              <p style={{ color: '#4A6A8A', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>Aucune fabrication en cours</p>
            )}
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0F2540' }}>✅ LIVRAISONS EFFECTUÉES</h3>
            <Link href="/dossiers-signes" style={{ fontSize: 12, color: '#7B4F2E', fontWeight: 600, textDecoration: 'none' }}>Voir tous →</Link>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {dossiersSignes.slice(0, 6).map(d => renderDossierItem(d as any))}
            {dossiersSignes.length === 0 && (
              <p style={{ color: '#4A6A8A', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>Aucune livraison effectuée</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 16, padding: '20px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 28 }}>
        <h3 style={{ margin: '0 0 16px', fontSize: 15, fontWeight: 700, color: '#0F2540' }}>📅 PLANNING ATELIER</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'auto repeat(7, 1fr)', gap: 0, fontSize: 12, overflowX: 'auto' }}>
          ...
          {DAYS_OF_WEEK.map(day => (
            <div key={day} style={{
              fontWeight: 700, color: '#7B4F2E', padding: '8px 4px',
        ...
        <div style={{ marginTop: 16, fontSize: 11, color: '#7A8E9F', textAlign: 'right' }}>
          <Link href="/planning" style={{ color: '#7B4F2E', fontWeight: 600, textDecoration: 'none' }}>
