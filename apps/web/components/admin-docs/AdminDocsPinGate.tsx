 ...
 *
 * Confidentialité supplémentaire au-delà de l'authentification (la page reste
 * protégée par le JWT côté serveur). C'est un « écran de veille » : quelqu'un qui
 * regarde par-dessus l'épaule ne peut pas ouvrir les documents sensibles.
 *
 * - Le code est stocké sur le COMPTE (useConfigStore → WorkspaceSettings.extra),
 *   donc synchronisé entre appareils et modifiable depuis Paramètres.
 * - 1ère utilisation → définition du code (saisie + confirmation).
 * - Ensuite → code demandé à chaque accès.
 * - « Code oublié ? » → réinitialisation protégée par le MOT DE PASSE du compte
 *   (endpoint /auth/verify-password, sans effet de bord).
 */
...
export function AdminDocsPinGate({ children }: { children: React.ReactNode }) {
  ...
  const [mode, setMode] = useState<Mode>(storedPin ? 'enter' : 'setup-new');
  const [buffer, setBuffer] = useState('');
  const [firstPin, setFirstPin] = useState('');
  const [error, setError] = useState('');
  const [shake, setShake] = useState(false);

  // Mot de passe (mode réinitialisation)
  const [pwd, setPwd] = useState('');
  const [pwdBusy, setPwdBusy] = useState(false);

  // Suit l'état stocké (ex. hydratation backend qui arrive après le montage) :
  // tant qu'on n'est pas en train de réinitialiser, on aligne le mode.
  useEffect(() => {
    if (unlocked) return;
  ...
  useEffect(() => {
    ...
    if (mode === 'enter') {
      ...
      if (code === firstPin) { setAdminDocsPin(code); setUnlocked(true); }
      else { setFirstPin(''); setMode('setup-new'); fail('Les deux codes ne correspondent pas. Recommencez.'); }
    }
  ...
  const subtitle =
    ...
    : mode === 'setup-new' ? 'Choisissez un code à 4 chiffres. Il vous sera demandé à chaque accès.'
    ...
    <div className="flex items-center justify-center py-10" style={{ minHeight: '70vh' }}>
      ...
      <div className={`w-full max-w-sm rounded-3xl bg-white p-8 text-center shadow-xl border border-[#304035]/8 ${shake ? 'avra-pin-shake' : ''}`}>
        <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#304035]">
          {isReset ? <KeyRound className="h-7 w-7 text-[#c9a96e]" /> : <Lock className="h-7 w-7 text-[#c9a96e]" />}
        </div>
          ...
          <>
            ...
            {mode === 'enter' ? (
              ...
              <p className="mt-6 inline-flex items-center gap-1.5 text-xs text-[#304035]/40">
                <ShieldCheck className="h-3.5 w-3.5" /> Synchronisé et chiffré côté serveur</p>
            )}
          </>
        )}
      </div>
