...
export function buildRenduFromImageKontextPrompt(params: RenduParams): BuiltPrompt {
  ...
  const prompt = [
    ...
    ``,
    `★★★ FLOOR — THE #1 ELEMENT TO PRESERVE (the model most often wrongly re-invents the floor):`,
    `Reproduce the floor with ABSOLUTE, LITERAL precision — it must be the SAME floor as the source: EXACT same material, EXACT same pattern, EXACT same plank/tile DIRECTION and layout, EXACT same color, EXACT same grout/joint lines and reflectivity. Wood parquet → keep wood parquet with the SAME plank orientation; tiles → keep the SAME tile size and grid alignment; concrete/microcement → keep it as-is. NEVER swap the floor material, NEVER change its pattern, color, scale or direction. Only photorealise its texture — never redesign it.`,
    ``,
    `Architecture & layout:`,
    ...
    `- DO NOT replace, recolor, re-pattern, rescale or change the plank/tile direction of the FLOOR (the single most common mistake) unless the user explicitly requested it above`,
