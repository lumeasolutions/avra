...
function computeAlerts(): RawAlert[] {
  ...
  // ── Dossiers INACTIFS : archivés / terminés / perdus ────────────────────────
  // Retour cofondatrice (sept. 2026) : « j'ai des alertes sur des dossiers
  // archivés — attention de plus avoir d'alerte sur dossiers archivés et perdus ».
  // `activeSignes` ne protégeait QUE les alertes construites à partir des
  // dossiers signés. Toutes les autres sources (factures, devis, paiements,
  // commandes, relances, demandes de devis) portent un `dossierId` sans jamais
  // vérifier l'état du dossier : une facture d'acompte ou une demande de devis
  // rattachée à un dossier archivé continuait donc à alerter indéfiniment.
  // On construit ici l'ensemble des dossiers à ignorer, et on filtre TOUTES les
  // alertes en une seule passe avant le tri — aucune source ne peut passer au
  // travers, y compris celles qu'on ajoutera plus tard.
  // On lit le store BRUT (pas la version filtrée par métier) : un dossier
  // archivé doit rester muet quel que soit le portail ouvert.
  const inactiveDossierIds = new Set<string>();
  for (const ds of rawDossiers.dossiersSignes ?? []) {
    if (ds.terminated || ds.archivedAt) inactiveDossierIds.add(ds.id);
  }
  for (const dp of rawDossiers.dossiersPerdus ?? []) {
    inactiveDossierIds.add(dp.id);
  }
  ...
  // #2 — Acompte non reçu (dossiers signés) — RETIRÉ (14/07/2026).
  // Demande produit : une « relance » sur un dossier DÉJÀ SIGNÉ n'a pas de sens
  // dans l'assistant (les relances client ne concernent que les dossiers EN
  // COURS, avant signature). On ne génère donc plus cette alerte.
  ...
  for (const cmd of commandes) {
    ...
    if (dateLiv && dateLiv < now) {
      alerts.push({
        ...
        autoGenerated: true,
        // « Appeler fournisseur » retiré (demande cofondatrice sept. 2026) —
        // on garde un accès au dossier pour que l'alerte reste actionnable.
        actions: cmd.dossierId
          ? [{ label: 'Voir dossier', href: `/dossiers/${cmd.dossierId}` }]
          : [],
      });
    }
  }
  ...
  // ── Filtre global : aucune alerte sur un dossier archivé / terminé / perdu ──
  // Une alerte sans `dossierId` (société non configurée, stock, planning,
  // intervenant…) n'est pas concernée : elle ne pointe vers aucun dossier.
  const visibles = inactiveDossierIds.size === 0
    ? alerts
    : alerts.filter((a) => !a.dossierId || !inactiveDossierIds.has(a.dossierId));

  // ── Trier par sévérité ──
  const order: Record<AlertSeverity, number> = { error: 0, warning: 1, clock: 2, info: 3 };
  visibles.sort((a, b) => order[a.severity] - order[b.severity]);
